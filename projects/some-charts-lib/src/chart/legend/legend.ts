...
export class Legend implements IDisposable {
    ...
    updateContent(legendItems: Array<LegendItem>) {
        if (!this.legendElt) {
            this.legendElt = $('<div></div>').addClass('sc-legend');

            this.legendElt.css('font-size', this.options.fontSize!);
        ...
        for (let legendItem of legendItems) {
            ...
            if (isString(legendItem.color)) {
                ...
                colorDiv.css(
                    'background',
                    `linear-gradient(to right, ${legendItem.color.range.min.toString()}, ${legendItem.color.range.max.toString()}`,
                );
            }
            ...
            let nameDiv = $(
                '<div class="sc-legend-item__caption">' +
                    legendItem.caption +
                    '</div>',
