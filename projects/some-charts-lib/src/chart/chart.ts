    ...
    implements EventListener<DataSetEventType | AnimationEventType>, IDisposable
{
    ...
    private _visibleRect:
        | DataRect<
              XDimensionType,
              YDimensionType extends undefined
                  ? number
    ...
    public get screenRect(): NumericDataRect {
        return (
            this._screenRect ??
            new NumericDataRect(0, this.size.width, 0, this.size.height)
        );
    }
    ...
    constructor(
        ...
        let rendererOptions = merge(
            cloneDeep(RendererOptionsDefaults.Instance),
        ...
        let plotOptionsArr = this.options
            .plots!.map((po) =>
                PlotOptionsClassFactory.buildPlotOptionsClass(po),
            )
        ...
        this.element =
            this.jqueryElt.length > 0 ? this.jqueryElt[0] : undefined;

        this._location = new NumericPoint(0, 0);
        this._size = new Size(
            this.jqueryElt.innerWidth() ?? 0,
            this.jqueryElt.innerHeight() ?? 0,
        );
        ...
        this.resizeSensor = this.element
            ? new ResizeSensor(this.element, this.resizeSensorCallback)
            : undefined;
        ...
        this.layersConfigs.push(
            ...Chart.getCommonLayersIds().map((layerId) => {
        ...
        this._visibleRectAsNumeric = visibleRect
            ? this.dimensionalVisibleRectToNumeric(visibleRect)
            : new NumericDataRect(0, 1, 0, 1);

        this.dataTransformation = new DataTransformation(
            CoordinateTransformationStatic.buildFromOptions(
                this.options?.axes!,
            ),
        );
        ...
        this.horizontalAxis = this.createAxis(
            this.dataTransformation,
            AxisOrientation.Horizontal,
            this.options?.axes!.horizontal,
        );
        this.verticalAxis = this.createAxis(
            this.dataTransformation,
        ...
        for (let plotOptions of plotOptionsArr) {
            let plot = this.plotFactory?.createPlot<
                TItemType,
                XDimensionType,
                YDimensionType
            >(dataSet, this.dataTransformation, plotOptions);
            if (plot) {
                ...
                for (let plotLayerId of plotLayers) {
                    if (
                        this.layersConfigs.findIndex(
                            (config) => config.id === plotLayerId,
                        ) < 0
                    ) {
        ...
        if (this.options.navigation!.isEnabled === true) {
            ...
            this.mouseNavigation = new MouseNavigation(
                this.location,
                this.size,
            );
    ...
    updateNumeric(
        ...
        this._visibleRect =
            this.numericVisibleRectToDimensional(visibleRectAsNumeric);
        ...
        if (this.verticalAxis) {
            this.verticalAxis.update(
                ...
                verticalRange,
                undefined,
                this._size.height - horizontalAxisHeight,
            );
        }
        ...
        if (this.horizontalAxis) {
            let axisRange =
                this.options.axes?.horizontal.axisType === AxisTypes.DateAxis
                    ? new Range<Date>(
                          DimensionValue.buildForDateFromPrimitive(
                              horizontalRange.min,
                          ).value,
                          DimensionValue.buildForDateFromPrimitive(
                              horizontalRange.max,
            ...
            this.horizontalAxis.update(
                new NumericPoint(
                    this._location.x + (verticalAxisSize?.width ?? 0),
                    this._location.y + (verticalAxisSize?.height ?? 0),
                ),
                axisRange,
                this._size.width - (verticalAxisSize?.width ?? 0),
                undefined,
            );
        }
        ...
        let gridLocation = new NumericPoint(
            this._location.x + (verticalAxisSize?.width ?? 0),
            this._location.y,
        );
        let gridSize = new Size(
            horizontalAxisSize?.width ?? this.size.width,
            verticalAxisSize?.height ?? this.size.height,
        );
        ...
        if (this.horizontalAxis) {
            horizontalAxisTicks =
                this.horizontalAxis.getMajorTicksScreenCoords();
        }
        ...
        this._screenRect = new NumericDataRect(
            gridLocation.x,
            gridLocation.x + gridSize.width,
            gridLocation.y,
            gridLocation.y + gridSize.height,
        );
            ...
            if (chartBoundingRect) {
                for (let i = 1; i < this.plots.length; i++) {
                    ...
                    if (plotBoundingRect) {
                        ...
                        chartFitToViewMargin = chartFitToViewMargin.merge(
                            plot.getFitToViewMargin(),
                        );
                    }
                }

                chartBoundingRect = this.addMarginToBoundingRect(
                    chartBoundingRect,
                    chartFitToViewMargin,
                );

                let coeff = 0.03;

                let horSize = chartBoundingRect
                    .getHorizontalRange()
                    .getLength();
                ...
                chartBoundingRect = new NumericDataRect(
                    ...
                    chartBoundingRect.maxX + offsetX / 2,
                    chartBoundingRect.minY - offsetY / 2,
                    chartBoundingRect.maxY + offsetY / 2,
                );

                if (this.options.navigation!.fixedTopBound) {
                    chartBoundingRect = chartBoundingRect.withHeight(
                        (this.options.navigation!.fixedTopBound -
                            chartBoundingRect.minY) *
                            (1 + coeff),
                    );
                } else if (this.options.navigation!.relativePadding) {
                    chartBoundingRect = chartBoundingRect.addRelativePadding(
                        this.options.navigation!.relativePadding,
                    );
                }
    ...
    private addMarginToBoundingRect(
        boundingRect: NumericDataRect,
        margin: Margin,
    ): NumericDataRect {
        return this.dataTransformation.screenRegionToDataForRect(
            this.screenRect.addMargin(margin),
            boundingRect,
            this.screenRect,
        );
    }
    ...
    private createAxis(
        ...
        let axisRange =
            orientation === AxisOrientation.Vertical
                ? this._visibleRectAsNumeric.getVerticalRange()
                : this._visibleRectAsNumeric.getHorizontalRange();
        ...
        if (options.axisType == AxisTypes.NumericAxis) {
            return new NumericAxis(
                ...
                axisRange,
                dataTransformation,
                options as NumericAxisOptions,
                axisWidth,
                axisHeight,
            );
        } else if (options.axisType === AxisTypes.LabeledAxis) {
            return new LabeledAxis(
                axisLocation,
                orientation,
                axisRange,
                dataTransformation,
            ...
            options.axisType === AxisTypes.DateAxis &&
    ...
    private static createLayers(
        renderer: Renderer,
        layersConfigs: Array<Konva.LayerConfig>,
    ) {
    ...
    protected updateLabeledAxesLabels() {
        const dataSetWithLabeledAxisMustHaveStringDimensionErrorMessage =
            'DataSet with labeled axis must have the corresponding dimension of string type!';

        if (this.options.axes!.horizontal.axisType === AxisTypes.LabeledAxis) {
            if (this.dataSet.dimensionXType === DimensionType.String) {
                (this.horizontalAxis as LabeledAxis).updateLabels(
                    this.dataSet.dimensionXValues.map(
                        (v) => <Point<string>>v.toPoint(),
                    ),
                );
            } else
                throw new Error(
                    dataSetWithLabeledAxisMustHaveStringDimensionErrorMessage,
                );
        }
        if (this.options.axes!.vertical.axisType === AxisTypes.LabeledAxis) {
            if (this.dataSet.is2D) {
                if (this.dataSet.dimensionXType === DimensionType.String) {
                    (this.verticalAxis as LabeledAxis).updateLabels(
                        this.dataSet.dimensionYValues!.map(
                            (v) => <Point<string>>v.toPoint(),
                        ),
                    );
                } else
                ...
                throw new Error(
                    "1D DataSet can't be used with horizontal labeled axis!",
                );
        }
    }
    ...
    private buildLegend(plotOptionsArr: Array<PlotOptionsClass>) {
        ...
        this.legend.updateContent(
            plotOptionsArr.flatMap((po) => {
    ...
    protected dimensionalVisibleRectToNumeric(
        visibleRect: DataRect<
            XDimensionType,
            YDimensionType extends undefined
                ? number
        ...
        return new NumericDataRect(
            ...
            this.dataSet.yDimensionValueToNumeric(
                visibleRect.minY as YDimensionType,
            ) ?? 0,
            this.dataSet.yDimensionValueToNumeric(
                visibleRect.maxY as YDimensionType,
            ) ?? 1,
        );
    }

    protected numericVisibleRectToDimensional(
        visibleRectAsNumeric: NumericDataRect,
    ):
        | DataRect<
              XDimensionType,
          ...
          >
        | undefined {
        let minXasDimVal = this.dataSet.numericToXDimensionValue(
            visibleRectAsNumeric.minX,
        );
        let minYasDimVal = this.dataSet.numericToYDimensionValue(
            visibleRectAsNumeric.minY,
        );
        let maxXasDimVal = this.dataSet.numericToXDimensionValue(
            visibleRectAsNumeric.maxX,
        );
        let maxYasDimVal = this.dataSet.numericToYDimensionValue(
            visibleRectAsNumeric.maxY,
        );

        return minXasDimVal !== undefined &&
            minYasDimVal !== undefined &&
            maxXasDimVal !== undefined &&
            maxYasDimVal !== undefined
            ? new DataRect<
                  XDimensionType,
                  YDimensionType extends undefined
                      ? number
              ...
              >(
                  minXasDimVal,
                  maxXasDimVal,
                  minYasDimVal as YDimensionType extends undefined
                      ? number
                      : Exclude<YDimensionType, undefined>,
                  maxYasDimVal as YDimensionType extends undefined
                      ? number
                      : Exclude<YDimensionType, undefined>,
              )
            : undefined;
