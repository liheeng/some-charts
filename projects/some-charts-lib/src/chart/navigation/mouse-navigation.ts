...
export class MouseNavigation extends ChartContent(Object) {
    ...
    private readonly onMouseDownTouchStartHandler: (
        event: JQuery.Event,
    ) => void;
            ...
            if (event.touches && event.targetTouches) {
                if (event.touches.length == 1) {
                    ...
                    for (let i = 0; i < event.touches.length; i++) {
                        ...
                        touchArray.push(
                            new NumericPoint(touch.pageX, touch.pageY),
                        );
                    }
    ...
    protected onMouseMove(e: JQuery.Event) {
        if (e.touches) {
            ...
            if (this.isMouseDown && this.prevPoint && e.pageX && e.pageY) {
                let curPoint = new NumericPoint(
                    e.pageX - this.location.x,
                    e.pageY - this.location.y,
                );
                this.handleDragging(
    ...
    private handleDragging(pixelDeltaX: number, pixelDeltaY: number) {
        if (this.chart) {
            let horizontalRange =
                this.chart.visibleRectAsNumeric.getHorizontalRange();
            let verticalRange =
                this.chart.visibleRectAsNumeric.getVerticalRange();

            let plotSize = this.chart.getPlotSize();

            let dataDeltaX =
                pixelDeltaX * (horizontalRange.getLength() / plotSize.width);
            let dataDeltaY =
                pixelDeltaY * (verticalRange.getLength() / plotSize.height);
            ...
            this.chart.updateNumeric(
                new NumericDataRect(
                    newHorizontalRange.min,
                    newHorizontalRange.max,
                    newVerticalRange.min,
                    newVerticalRange.max,
                ),
                true,
            );
        }
    ...
    private handleMultitouchZooming(
        prevPixRect: NumericDataRect,
        curPixRect: NumericDataRect,
    ) {
        if (this.chart) {
            let plotSize = this.chart.getPlotSize();
            ...
            let rightBottomDeltaX =
                -(curPixRect.maxX - prevPixRect.maxX) * propX;
            let rightBottomDeltaY =
                (curPixRect.maxY - prevPixRect.maxY) * propY;

            let newHorizontalRange = new NumericRange(
                horizontalRange.min + leftTopDeltaX,
                horizontalRange.max + rightBottomDeltaX,
            );
            let newVerticalRange = new NumericRange(
                verticalRange.min + rightBottomDeltaY,
                verticalRange.max + leftTopDeltaY,
            );

            let dataRange = new NumericDataRect(
                newHorizontalRange.min,
                newHorizontalRange.max,
                newVerticalRange.min,
                newVerticalRange.max,
    ...
    private handleScrollEvent(delta: number) {
        if (this.chart) {
            ...
            let newHorRange = new NumericRange(
                horRange.min + deltaX,
                horRange.max - deltaX,
            );
            let newVerRange = new NumericRange(
                verRange.min + deltaY,
                verRange.max - deltaY,
            );

            if (
                newHorRange.max - newHorRange.min >= 1e-8 &&
                newVerRange.max - newVerRange.min >= 1e-8
            ) {
                this.chart.updateNumeric(
                    new NumericDataRect(
                        newHorRange.min,
    ...
    override placeOnChart(chart?: Chart) {
        ...
        if (chart) {
            ...
            this.container.on(
                ...
                this.onMouseDownTouchStartHandler,
            );
            this.container.on(
                'mouseup touchend',
                this.onMouseUpTouchEndHandler,
            );
