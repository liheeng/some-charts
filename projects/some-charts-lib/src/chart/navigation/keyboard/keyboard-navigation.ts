    ...
    protected onKeyDown(e: JQuery.Event) {
        if (this.chart && this.host && this.isHostFocused) {
            ...
            switch (e.key) {
                ...
                case 'ArrowLeft':
                    horizontalRange =
                        horizontalRange.withShift(-horizontalStep);
                    break;
            ...
            if (
                horizontalRange.max - horizontalRange.min >=
                    this.chart.minZoomLevel &&
                verticalRange.max - verticalRange.min >= this.chart.minZoomLevel
            ) {
                let newChartDataRect = new NumericDataRect(
                    horizontalRange.min,
                    horizontalRange.max,
                    verticalRange.min,
                    verticalRange.max,
                );
                this.chart.updateNumeric(newChartDataRect, true);
            }
