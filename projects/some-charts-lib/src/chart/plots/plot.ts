...
export abstract class Plot<
    ...
    >
    extends ChartRenderableItem<Konva.Group>
    implements EventListener<DataSetEventType | AnimationEventType>, IDisposable
{
    ...
    private metricPoints1DMap!: Map<
        string,
        Array<NumericPoint | Array<NumericPoint>>
    >;
        ...
        this.shapesGroup = new Konva.Group({
            clipFunc: (context) => {
                if (this.visible && this.screen) {
                    ...
                    context.rect(
                        screenLocation.x + 0.5,
                        screenLocation.y + 0.5,
    ...
    protected buildPlotOptionsClass(
        plotOptions: PlotOptionsType,
    ): PlotOptionsClassType {
        return PlotOptionsClassFactory.buildPlotOptionsClass(
            plotOptions,
        ) as PlotOptionsClassType;
    }
    ...
    eventCallback(
        ...
        if (event.type === DataSetEventType.Changed) {
            let changedEvent = event as DataSetChangedEvent<
                XDimensionType,
                YDimensionType
            >;
    ...
    protected updateFromDataSet(
        dataSetChange: DataSetChange<XDimensionType, YDimensionType>,
    ) {
    ...
    updateVisibleAndScreen(visible: NumericDataRect, screen: NumericDataRect) {
        ...
        this.screen = screen;
        if (this.visible && this.screen) {
            this.updateShapesVisibleAndScreen(
    ...
    protected abstract rebuildShapesFromDataSet(
        dataSetChange: DataSetChange<XDimensionType, YDimensionType>,
    ): void;

    protected abstract updateShapesVisibleAndScreen(
        dataTransformation: DataTransformation,
    ...
    protected getColor(
        ...
        if (this.dataSet.isSingleMetricValue(color.metricId)) {
            let metricValue = this.dataSet.getMetricValueForDimensions(
                color.metricId,
                xDimVal,
                yDimVal,
            ) as number;
            return new Transition<Color>(color.range).apply(
                this.dataSet.getMetricRange(color.metricId),
                metricValue,
            );
        } else
            throw new Error(
                'Color transition is only supported for single value metric',
            );
    }

    protected getDependantNumericValueForMetricValue(
        dependant: MetricDependantValue<number>,
        xDimVal: DimensionValue<XDimensionType>,
        ...
        if (this.dataSet.isSingleMetricValue(dependant.metricId)) {
            ...
            return metricValue
                ? new Transition<number>(dependant.range).apply(
                      this.dataSet.getMetricRange(dependant.metricId),
                      metricValue,
                  )
                : undefined;
        } else
            throw new Error(
                'Value transition is only supported for single value metric',
            );
    }

    protected getMetricPoints1D(
        metricId: string,
    ): Array<NumericPoint | Array<NumericPoint>> | undefined {
        if (!this.metricPoints1DMap.has(metricId)) {
            let dimensionXValues = this.dataSet.dimensionXValues;

            if (this.dataSet.is1D) {
                let metricValues = this.dataSet.getMetricValues(metricId) as
                    | number[]
                    | number[][];
                let points = dimensionXValues.map((dimXVal, index) => {
                    ...
                    if (Array.isArray(metricValuesForDimVal)) {
                        return (metricValuesForDimVal as number[]).map(
                            (metricValue) =>
                                new NumericPoint(
                                    dimXVal.toNumericValue(),
                                    metricValue,
                                ),
                        );
                    } else
                        return new NumericPoint(
                            dimXVal.toNumericValue(),
                            metricValuesForDimVal as number,
                        );
                });
