...
> {
    ...
    override buildPlotOptionsClass(
        plotOptions: BarsPlotOptions,
    ): BarsPlotOptionsClass {
        return PlotOptionsClassFactory.buildPlotOptionsClass(
            merge(cloneDeep(BarsPlotOptionsDefaults.Instance), plotOptions),
        ) as BarsPlotOptionsClass;
    }

    override init(plotOptionsClass: BarsPlotOptionsClass) {
        super.init(plotOptionsClass);
        this.colorsByMetricId = new Map(
            this.plotOptions.metrics.map((m) => [
                m.id,
                this.getBarsColoring(m.color),
            ]),
        );
    ...
    protected add1DPlotElements(
        ...
        for (
            let metricIdx = this.plotOptions.metrics.length - 1;
            metricIdx >= 0;
            metricIdx--
        ) {
            ...
            let prevMetric =
                metricIdx !== 0
                    ? this.plotOptions.metrics[metricIdx - 1]
                    : undefined;

            let metricValue = this.dataSet.getSingleMetricValue(
                metric.id,
                xDimVal.value,
            );
            ...
            if (metricValue) {
                ...
                let pointLocation = this.getSingleMetricPoint1D(
                    metric.id,
                    xDimVal,
                );
                let rect = this.getBarRectForMetric(
                    metric.id,
                    prevMetric?.id,
                    xDimVal,
                );
                ...
                if (pointLocation && rect) {
                    let labelText: string | undefined = this.plotOptions
                        .drawLabelsOnBars
                        ? metricValue.toFixed(this.plotOptions.labelsPrecision)
                        : undefined;
    ...
    protected update1DPlotElement(
        ...
        let metricIdx = this.plotOptions.metrics.findIndex(
            (m) => m.id === plotElt.metricId,
        )!;
        let metric = this.plotOptions.metrics[metricIdx];
        let prevMetric =
            metricIdx !== 0
    ...
    private getBarRectForMetric(
        metricId: string,
        prevMetricId: string | undefined,
        xDimVal: DimensionValue<XDimensionType>,
        ...
        if (pointLocation && barWidth) {
            if (!prevMetricId) {
                ...
                let prevPointLocation = this.getSingleMetricPoint1D(
                    prevMetricId,
                    xDimVal,
                );
                ...
                if (prevPointLocation) {
                    rectX = MathHelper.optimizeValue(-barWidth / 2);
                    rectY = -MathHelper.optimizeValue(
                        pointLocation.y - prevPointLocation.y,
                    );
                    rectW = MathHelper.optimizeValue(barWidth);
                    rectH = MathHelper.optimizeValue(
                        pointLocation.y - prevPointLocation.y,
                    );
                }
            }
    ...
    private getBarsColoring(fill: Color): BarsColoring {
        let fromFillColor = new ColorObj(fill)
            .lighten(0.3)
            .rgb()
            .hex() as Color;
        ...
        else strokeColor = new ColorObj(fill).darken(0.3).rgb().hex() as Color;

        return new BarsColoring(
            new Range<Color>(fromFillColor, toFillColor),
            strokeColor,
        );
    }
