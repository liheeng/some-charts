...
export class Box extends PlotDrawableElement<Konva.Group> {
    ...
    constructor(
        ...
        let boxCenter = Box.calculateBoxCenter(
            dataPoints,
            percentile25,
            percentile75,
        );
        ...
        this.relative25Percentile = new AnimatedProperty<number>(
            PercentileHelper.calculate25Percentile(dataPoints) - boxCenter.y,
        );
        this.relative75Percentile = new AnimatedProperty<number>(
            PercentileHelper.calculate75Percentile(dataPoints) - boxCenter.y,
        );
        this.relative50Percentile = new AnimatedProperty<number>(
            PercentileHelper.calculate50Percentile(dataPoints) - boxCenter.y,
        );
        this.relativeMinY = new AnimatedProperty<number>(
            PercentileHelper.calculateMinY(percentile25, percentile75) -
                boxCenter.y,
        );
        this.relativeMaxY = new AnimatedProperty<number>(
            PercentileHelper.calculateMaxY(percentile25, percentile75) -
                boxCenter.y,
        );
        ...
        this.topWhiskerShape = new Konva.Shape({
            ...
            sceneFunc: (context: Konva.Context, shape: Konva.Shape) => {
                ...
                context.moveTo(
                    -shape.getAttr('whiskersRenderWidth') / 2,
                    shape.getAttr('relativeMaxY'),
                );
                context.lineTo(
                    shape.getAttr('whiskersRenderWidth') / 2,
                    shape.getAttr('relativeMaxY'),
        ...
        this.avgLineShape = new Konva.Shape({
            ...
            sceneFunc: (context: Konva.Context, shape: Konva.Shape) => {
                ...
                context.beginPath();
                context.moveTo(
                    -shape.getAttr('boxRenderWidth') / 2,
                    shape.getAttr('relativeAvg'),
                );
                context.lineTo(
                    shape.getAttr('boxRenderWidth') / 2,
                    shape.getAttr('relativeAvg'),
    ...
    public setDataPoints(
        ...
        this.relative25Percentile.setValue(
            PercentileHelper.calculate25Percentile(dataPoints) - boxCenter.y,
            animate,
            animationDuration,
        );
        this.relative75Percentile.setValue(
            PercentileHelper.calculate75Percentile(dataPoints) - boxCenter.y,
            animate,
            animationDuration,
        );
        this.relative50Percentile.setValue(
            PercentileHelper.calculate50Percentile(dataPoints) - boxCenter.y,
            animate,
            animationDuration,
        );
        this.relativeMinY.setValue(
            PercentileHelper.calculateMinY(percentile25, percentile75) -
                boxCenter.y,
            animate,
            animationDuration,
        );
        this.relativeMaxY.setValue(
            PercentileHelper.calculateMaxY(percentile25, percentile75) -
                boxCenter.y,
            animate,
            animationDuration,
    ...
    override updateShapesForAnimationFrame(
        ...
        let relative25PercentileOnScreen =
            dataTransformation.getRelativeYValueLocationOnScreen(
                dataPoint,
                relative25Percentile,
                visible,
                screen,
            );
        let relative50PercentileOnScreen =
            dataTransformation.getRelativeYValueLocationOnScreen(
                dataPoint,
                relative50Percentile,
                visible,
                screen,
            );
        let relative75PercentileOnScreen =
            dataTransformation.getRelativeYValueLocationOnScreen(
                dataPoint,
                relative75Percentile,
                visible,
                screen,
            );

        let relativeMinYOnScreen =
            dataTransformation.getRelativeYValueLocationOnScreen(
                dataPoint,
                relativeMinY,
                visible,
        ...
        let boxXMin = dataTransformation.getRelativeXValueLocationOnScreen(
            dataPoint,
            dataPoint.x - this.boxDataWidth / 2,
            visible,
            screen,
        );
        let boxXMax = dataTransformation.getRelativeXValueLocationOnScreen(
            dataPoint,
            dataPoint.x + this.boxDataWidth / 2,
            visible,
            screen,
        ...
        let whiskersXMin = dataTransformation.getRelativeXValueLocationOnScreen(
            dataPoint,
            dataPoint.x - this.whiskersDataWidth / 2,
            visible,
            screen,
        );
        let whiskersXMax = dataTransformation.getRelativeXValueLocationOnScreen(
            dataPoint,
            dataPoint.x + this.whiskersDataWidth / 2,
            visible,
            screen,
    ...
    private static calculateBoxCenter(
        dataPoints: Array<NumericPoint>,
        percentile25: number,
        percentile75: number,
    ) {
