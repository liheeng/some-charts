...
> {
    ...
    override buildPlotOptionsClass(
        ...
        return PlotOptionsClassFactory.buildPlotOptionsClass(
            merge(cloneDeep(BoxOutliersPlotOptionsDefaults.Instance), options),
        ) as BoxOutliersPlotOptionsClass;
    }
    ...
    override add1DPlotElements(
        ...
        let metricPoints = this.dataSet
            .getArrayMetricValue(metricId, xDimVal.value)!
            .filter((mv) => mv < statRange.min || mv > statRange.max)
            .map((y) => new NumericPoint(xDimVal.toNumericValue(), y));

        return metricPoints.length !== 0
            ? [new BoxOutliers(metricId, metricPoints, markerColor, markerSize)]
            : [];
    }

    override add2DPlotElements(
        xDimVal: DimensionValue<XDimensionType>,
    ...
    override update1DPlotElement(
        ...
        if (markerColor && markerSize && metricValues) {
            boxOutliers.setDataPoints(
                metricValues.map(
                    (mv) => new NumericPoint(xDimVal.toNumericValue(), mv),
                ),
    ...
    private getYStatisticalRange(
        metricId: string,
        xDimVal: DimensionValue<XDimensionType>,
    ): NumericRange {
        if (!this.statRangeByMetricId.has(metricId)) {
            this.statRangeByMetricId.set(
                metricId,
                new Map<number, NumericRange>(),
            );
        }

        let range = this.statRangeByMetricId
            .get(metricId)!
            .get(xDimVal.toNumericValue());
        ...
        if (!range) {
            ...
            let percentile25 =
                PercentileHelper.calculate25Percentile(metricPoints);
            let percentile75 =
                PercentileHelper.calculate75Percentile(metricPoints);

            let minY = PercentileHelper.calculateMinY(
                percentile25,
                percentile75,
            );
            let maxY = PercentileHelper.calculateMaxY(
                percentile25,
                percentile75,
            );
            ...
            this.statRangeByMetricId
                .get(metricId)!
                .set(xDimVal.toNumericValue(), range);
        }

        return range;
    }

    override update2DPlotElement(
        plotElt: PlotDrawableElement,
        xDimVal: DimensionValue<XDimensionType>,
        yDimVal: DimensionValue<Exclude<YDimensionType, undefined>>,
    ) {
