...
export class BoxOutliers extends PlotDrawableElement {
    ...
    constructor(
        ...
        this.outliersRelativePositions = dataPoints.map(
            (dp) =>
                new AnimatedProperty<NumericPoint>(
                    dp.scalarPlus(origin.additiveInvert()),
                ),
        );
        this._color = color;
        this._size = size;
        this.outliersShapes = this.outliersRelativePositions.map((pos) =>
            BoxOutliers.createMarkerShape(color, size, pos.displayedValue),
        );
    ...
    public setDataPoints(
        ...
        if (dataPoints.length !== 0) {
            let origin = BoxOutliers.getOrigin(dataPoints);
            this.dataPoint.setValue(origin, animate, animationDuration);
            if (this.outliersRelativePositions.length !== dataPoints.length) {
                ...
                this.outliersShapes = this.outliersRelativePositions.map(
                    (pos) =>
                        BoxOutliers.createMarkerShape(
                            this.color,
                            this.size,
                            pos.displayedValue,
                        ),
                ...
                this.outliersShapes.forEach((shape) =>
                    (this.rootDrawable as Konva.Group).add(shape),
                );
            } else {
                zip(
                    ...
                    relativePosition?.setValue(
                        dataPoint?.scalarPlus(origin.additiveInvert())!,
                        animate,
                        animationDuration,
                    );
                    shape!.setAttrs({
    ...
    override updateShapesForAnimationFrame(
        ...
        zip(this.outliersShapes, this.outliersRelativePositions).map(
            ([shape, position]) => {
                let positionOnScreen =
                    dataTransformation.getRelativePointLocationOnScreen(
                        dataPoint,
                        position!.displayedValue,
                        visible,
                        screen,
                    );
                shape!.setAttrs({
    ...
    private static getOrigin(dataPoints: Array<NumericPoint>): NumericPoint {
        if (dataPoints.length === 0) {
            throw new Error(
                'Failed to get box outliers origin. DataPoints array is empty!',
            );
        }
