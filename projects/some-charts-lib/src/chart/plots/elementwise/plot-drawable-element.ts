...
> implements IDisposable
{
    ...
    constructor(
        ...
        public readonly rootDrawable: DrawableType,
    ) {
    ...
    updateShapes(
        ...
        let animationsIds = this.animatedProperties
            .map((p) => p.animationId)
            .filter((v) => v !== undefined) as Array<number>;

        this.updateRootDrawableRenderLocation(
            this.dataPoint.displayedValue,
            dataTransformation,
            visible,
            screen,
        );
        this.updateShapesForAnimationFrame(
            this.dataPoint.displayedValue,
            dataTransformation,
            visible,
            screen,
        ...
        if (
            this.isAnimationInProcess &&
            !isEqual(this.runningAnimationsIds, animationsIds)
        ) {
    ...
    protected updateShapesForAnimationFrame(
        dataPoint: NumericPoint,
        dataTransformation: DataTransformation,
        visible: NumericDataRect,
        screen: NumericDataRect,
    ...
    private updateRootDrawableRenderLocation(
        dataPoint: NumericPoint,
        dataTransformation: DataTransformation,
        visible: NumericDataRect,
        screen: NumericDataRect,
    ): void {
        let screenLocation = dataTransformation.dataToScreenRegionXY(
            dataPoint,
            visible,
            screen,
        );
