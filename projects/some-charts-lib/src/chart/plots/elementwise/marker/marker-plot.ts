...
export class MarkerPlot<
    ...
    MarkerPlotOptionsClassType extends
        MarkerPlotOptionsClass = MarkerPlotOptionsClass,
> extends ElementwisePlot<
    MarkerPlotOptionsType,
    MarkerPlotOptionsClassType,
    TItemType,
    XDimensionType,
    YDimensionType
> {
    constructor(
        ...
        this.plotOptions = PlotOptionsClassFactory.buildPlotOptionsClass(
            merge(cloneDeep(MarkerPlotOptionsDefaults.Instance), options),
        ) as MarkerPlotOptionsClassType;
    }

    protected static createElementForMarker(
        metricId: string,
    ...
    protected getMarkerSize(
        xDimVal: DimensionValue<XDimensionType>,
        yDimVal:
            | DimensionValue<Exclude<YDimensionType, undefined>>
            | undefined = undefined,
    ): number {
        return typeof this.plotOptions.markerSize === 'number'
            ? this.plotOptions.markerSize
            : this.getDependantNumericValueForMetricValue(
                  this.plotOptions.markerSize,
                  xDimVal,
                  yDimVal,
              )!;
    ...
    protected add1DPlotElements(
        ...
        return [
            MarkerPlot.createElementForMarker(
                metricId,
                point,
                markerColor,
                markerSize,
    ...
    protected add2DPlotElements(
        ...
        let markerColor = this.getColor(
            this.plotOptions.metric.color,
            xDimVal,
            yDimVal,
        );
        let markerSize = this.getMarkerSize(xDimVal, yDimVal);

        let point = new NumericPoint(
            xDimVal.toNumericValue(),
            yDimVal.toNumericValue(),
        );
        return [
            MarkerPlot.createElementForMarker(
                metricId,
    ...
    protected update1DPlotElement(
        ...
        if (markerColor && markerSize && metricValue) {
            marker.dataPoint.setValue(
                new NumericPoint(marker.dataPoint.actualValue.x, metricValue),
