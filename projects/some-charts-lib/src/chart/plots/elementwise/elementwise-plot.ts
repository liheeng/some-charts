...
> {
    ...
    protected rebuildShapesFromDataSet(
    ...
    private updatePlotElements(
        ...
        if (is2D) {
            let dataSetChange2D = dataSetChange as DataSetChange2D<
                XDimensionType,
                YDimensionType
            >;
            ...
            for (let i = 0; i < this.plotElements.length; i++) {
                ...
                if (
                    dataSetChange2D.isDeleted(
                        plotElt.dataPoint.actualValue.x,
                        plotElt.dataPoint.actualValue.y,
                    )
                ) {
                    deleted.push(plotElt);
                    deletedIndexes.push(i);
                } else if (
                    dataSetChange2D.isUpdated(
                        plotElt.dataPoint.actualValue.x,
                        plotElt.dataPoint.actualValue.y,
                    )
                ) {
                    let xy = dataSetChange2D.getUpdated(
                        plotElt.dataPoint.actualValue.x,
                        plotElt.dataPoint.actualValue.y,
                    )!;
            ...
            let dataSetChange1D =
                dataSetChange as DataSetChange1D<XDimensionType>;

            for (let i = 0; i < this.plotElements.length; i++) {
                let plotElt = this.plotElements[i];
                if (
                    dataSetChange1D.isDeleted(plotElt.dataPoint.actualValue.x)
                ) {
                    deleted.push(plotElt);
                    deletedIndexes.push(i);
                } else if (
                    dataSetChange1D.isUpdated(plotElt.dataPoint.actualValue.x)
                ) {
                    this.update1DPlotElement(
                        plotElt,
                        dataSetChange1D.getUpdated(
                            plotElt.dataPoint.actualValue.x,
                        )!,
                    );
                }
    ...
    protected abstract add1DPlotElements(
        xDimVal: DimensionValue<XDimensionType>,
    ): PlotDrawableElement[];

    protected abstract update1DPlotElement(
        plotElt: PlotDrawableElement,
        xDimVal: DimensionValue<XDimensionType>,
    ): void;

    protected abstract add2DPlotElements(
        xDimVal: DimensionValue<XDimensionType>,
        yDimVal: DimensionValue<Exclude<YDimensionType, undefined>>,
    ): PlotDrawableElement[];

    protected abstract update2DPlotElement(
        plotElt: PlotDrawableElement,
        xDimVal: DimensionValue<XDimensionType>,
        yDimVal: DimensionValue<Exclude<YDimensionType, undefined>>,
    ): void;

    protected updateShapesVisibleAndScreen(
        dataTransformation: DataTransformation,
    ...
    getBoundingRectangle(): NumericDataRect | undefined {
        return this.plotElements
            .map((plot) => plot.getBoundingRectangle())
