...
import {
    MajorDateTicksGenerator,
    MinorDateTicksGenerator,
} from '../ticks/date';
...
export class DateAxis extends AxisBase<Date, AxisOptions> {
    ...
    protected createMinorTicksGenerator():
        | MinorTicksGenerator<Date>
        | undefined {
        return new MinorDateTicksGenerator(
            this.options?.minorTickHeight!,
            this.majorTicksGenerator as MajorDateTicksGenerator,
        );
    }
        ...
        let renderHeight =
            ...
            2 *
                (TextMeasureUtils.measureFontHeight(this.options?.font!) +
                    2 +
                    DateAxis.paddingBetweenMajorAndMinorTicks);
    ...
    protected override renderHorizontalMinorTicks(context: Konva.Context) {
        if (this.minorTicks !== undefined) {
            ...
            for (let i = 0; i < minorTicksCount; i++) {
                ...
                context.fillText(tick.toString(), xVal, yVal);
            }
        }
    }
}
