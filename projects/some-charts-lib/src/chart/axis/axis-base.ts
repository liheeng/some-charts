...
> extends ChartRenderableItem<Konva.Shape> {
    /**
    ...
    protected constructor(
        ...
        this.ticksShape = new Konva.Shape({
            ...
            sceneFunc: (context: Konva.Context, shape: Konva.Shape) => {
                ...
                let axisRenderOriginX = MathHelper.optimizeValue(
                    this.location.x,
                );
                let axisRenderOriginY = MathHelper.optimizeValue(
                    this.location.y,
                );

                let axisRenderWidth = MathHelper.optimizeValue(
                    this._size.width,
                );
                let axisRenderHeight = MathHelper.optimizeValue(
                    this._size.height,
                );
                ...
                context.setAttr(
                    'font',
                    FontHelper.fontToString(this.options?.font!),
                );
                ...
                if (this.orientation == AxisOrientation.Horizontal) {
                    for (let i = 0; i < majorTicksCount; i++) {
                        ...
                        context.fillText(
                            ...
                            this.location.y +
                                tick.length -
                                labelSize.height *
                                    AxisBase.textVerticalOffsetMultiplier,
                        );

                        let xVal = MathHelper.optimizeValue(
                            this.location.x + tickScreenCoord,
                        );
                        let yVal = MathHelper.optimizeValue(this.location.y);
                        context.moveTo(xVal, yVal);
                        yVal = MathHelper.optimizeValue(
                            this.location.y + tick.length,
                        );
                    ...
                    let labelVerticalDelimiter =
                        2 - 2 * AxisBase.textVerticalOffsetMultiplier;
                    ...
                    for (let i = 0; i < majorTicksCount; i++) {
                        ...
                        context.fillText(
                            ...
                            this.location.y +
                                tickScreenXCoord -
                                labelSize.height / labelVerticalDelimiter,
                        );

                        let xVal = MathHelper.optimizeValue(
                            this.location.x + this._size.width - tick.length,
                        );
                        let yVal = MathHelper.optimizeValue(
                            this.location.y + tickScreenXCoord,
                        );
                        context.moveTo(xVal, yVal);
                        xVal = MathHelper.optimizeValue(
                            this.location.x + this._size.width,
                        );
                        context.lineTo(xVal, yVal);
                    }
    ...
    protected renderHorizontalMinorTicks(context: Konva.Context) {
        if (this.minorTicks !== undefined) {
            ...
            for (let i = 0; i < minorTicksCount; i++) {
                ...
                let xVal = MathHelper.optimizeValue(
                    this.location.x + ticksScreenXCoord,
                );
                let yVal = MathHelper.optimizeValue(this.location.y);
                context.moveTo(xVal, yVal);
    ...
    protected renderVerticalMinorTicks(context: Konva.Context) {
        if (this.minorTicks !== undefined) {
            ...
            for (let i = 0; i < minorTicksCount; i++) {
                ...
                let yVal = MathHelper.optimizeValue(
                    this.location.y + tickSceenXCoord,
                );
                context.moveTo(xVal, yVal);
                xVal = MathHelper.optimizeValue(
                    this.location.x + this._size.width,
    ...
    protected validateAxisInitialWidth() {
        if (
            ...
            throw new Error(
                'Undefined width is supported for vertical axis only',
            );
        }
    }
    ...
    protected validateAxisInitialHeight() {
        if (
            ...
            throw new Error(
                'Undefined height is supported for horizontal axis only',
            );
        }
    }

    protected abstract createMajorTicksGenerator(): MajorTicksGenerator<DataType>;
    protected abstract createMinorTicksGenerator():
        | MinorTicksGenerator<DataType>
        | undefined;
     ...
     */
    protected getTickScreenCoordinate(
        tick: Tick<DataType>,
        screenWidth: number,
        screenHeight: number,
        range: Range<DataType>,
        ...
        let numericRange = new NumericRange(
            this.axisValueToNumber(range.min),
            this.axisValueToNumber(range.max),
        );

        if (this.orientation == AxisOrientation.Horizontal)
            return this.dataTransformation.dataToScreenX(
                this.axisValueToNumber(tick.value),
                numericRange,
                screenWidth,
            );
        else
            return this.dataTransformation.dataToScreenY(
                this.axisValueToNumber(tick.value),
                numericRange,
                screenHeight,
            );
     ...
     * @returns {Array<Size>}
     * */
    protected measureLabelsSizesForMajorTicks(
        ticks: Array<Tick<DataType>>,
    ): Array<Size> {
        let labelsSizes = [];
    ...
    protected measureLabelSize(label: string): Size {
        let width = TextMeasureUtils.measureTextWidth(
            FontHelper.fontToString(this.options?.font!),
            label,
        );
    ...
    protected updateMajorTicks(range: Range<DataType>, size: Size) {
        this.majorTicks = this.generateMajorTicks(range, size);
        this.majorTicksLabelsSizes = this.measureLabelsSizesForMajorTicks(
            this.majorTicks,
        );
    }

    protected updateMinorTicks(range: Range<DataType>) {
        this.minorTicks = this.minorTicksGenerator?.generateTicks(
            range,
            this.majorTicks,
        );
    }

    protected updateTicksScreenCoords(
        location: NumericPoint,
        range: Range<DataType>,
        size: Size,
    ) {
        ...
        for (let tick of this.majorTicks) {
            majorTicksScreenCoords.push(
                this.getTickScreenCoordinate(
                    tick,
                    size.width,
                    size.height,
                    range,
    ...
    protected updateAxisSize() {
        ...
        if (
            ...
            renderHeight =
                TextMeasureUtils.measureFontHeight(this.options?.font!) +
                this.options?.majorTickHeight! +
                2;
        }
    ...
    protected generateMajorTicks(
        ...
        while (layout != LabelsLayout.OK) {
            if (attempt++ >= AxisBase.generateMajorTicksMaxAttempts) {
                ...
                if (range.isPoint()) ticksCount = 1;

                ticks = this.majorTicksGenerator.generateTicks(
                    this.range,
                    ticksCount,
                );
                ...
                if (ticks.length == prevTicksArrLength) {
                    ...
                    let labelsSizes =
                        this.measureLabelsSizesForMajorTicks(ticks);

                    prevLayout = layout;

                    layout = this.evaluateLabelsLayout(
                        this._size,
                        labelsSizes,
                        ticks,
                        range,
                    ...
                    if (
                        prevLayout == LabelsLayout.TooClose &&
                        layout == LabelsLayout.TooFar
                    ) {
                        layout = LabelsLayout.OK;
                    }
                    if (layout != LabelsLayout.OK) {
                        ...
                        if (layout == LabelsLayout.TooClose) {
                            ticksCount =
                                this.majorTicksGenerator.suggestDecreasedTickCount(
                                    ticksCount,
                                );
                        } else {
                            ticksCount =
                                this.majorTicksGenerator.suggestIncreasedTicksCount(
                                    ticksCount,
                                );
                        }
     ...
     * */
    protected evaluateLabelsLayout(
        axisSize: Size,
        ticksLabelsSizes: Array<Size>,
        ticks: Array<Tick<DataType>>,
        range: Range<DataType>,
    ): LabelsLayout {
        ...
        let ticksRenderInfo: Array<{ coord: number; length: number }> = flow(
            partialRight(
                map,
                (sizeTickTuple: { tick: Tick<DataType>; labelSize: Size }) => {
                    return {
                        coord: this.getTickScreenCoordinate(
                            sizeTickTuple.tick,
                            axisSize.width,
                            axisSize.height,
                            range,
                        ),
                        length: isAxisHorizontal
                            ? sizeTickTuple.labelSize.width
                            : sizeTickTuple.labelSize.height,
                    };
                },
            ),
            partialRight(
                sortBy,
                (i: { coord: number; length: number }) => i.coord,
            ),
        )(
            zipWith(ticksLabelsSizes, ticks, (size, tick) => {
                return { tick: tick, labelSize: size };
            }),
        );
        ...
        for (
            let l = 0, r = 1;
            l < ticksRenderInfo.length, r < ticksRenderInfo.length - 1;
            l++, r++
        ) {
            ...
            if (
                rightTick.coord - leftTick.coord <
                (leftTick.length / 2 + rightTick.length / 2) * 2
            ) {
                res = LabelsLayout.TooClose;
                break;
            } else if (
                rightTick.coord - leftTick.coord >
                (leftTick.length / 2 + rightTick.length / 2) * 1.5
            ) {
