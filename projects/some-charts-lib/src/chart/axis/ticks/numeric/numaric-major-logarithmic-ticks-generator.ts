...
export class NumericMajorLogarithmicTicksGenerator extends MajorTicksGenerator<number> {
    ...
    generateTicks(
        ...
        let firstTickValue = Math.floor(
            MathHelper.logByBase(this.logarithmBase, range.min),
        );
        let lastTickValue = Math.ceil(
            MathHelper.logByBase(this.logarithmBase, range.max),
        );
