...
export class NumericMinorTicksGenerator implements MinorTicksGenerator<number> {
    ...
    generateTicks(
        ...
        if (majorTicks.length < 2) {
            ...
            let step =
                (majorTicks[1].value - majorTicks[0].value) /
                (this.stepsCount - 1);

            let firstTick = majorTicks[0];
            let minBound =
                firstTick.value < range.min ? range.min : firstTick.value;

            let lastTick = majorTicks[majorTicks.length - 1];
            let maxBound =
                lastTick.value > range.max ? range.max : lastTick.value;
