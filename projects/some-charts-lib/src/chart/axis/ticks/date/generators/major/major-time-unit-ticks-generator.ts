    ...
    generateTicks(range: Range<Date>, ticksCount: number): Array<Tick<Date>> {
        ...
        switch (this.timeUnit) {
            ...
            case TimeUnit.Milliseconds:
                tickSpacing = Math.ceil(
                    MathHelper.calcNiceNumber(
                        niceDelta / (ticksCount - 1),
                        true,
                    ),
                );
                break;
        ...
        if (
            this.timeUnit === TimeUnit.Years ||
            this.timeUnit === TimeUnit.Months ||
            this.timeUnit === TimeUnit.Days
        ) {
            niceMin = zero
                .clone()
                .add(
                    Math.floor(min.diff(zero, this.timeUnit) / tickSpacing) *
                        tickSpacing,
                    this.timeUnit,
                );
            niceMax = zero
                .clone()
                .add(
                    Math.ceil(max.diff(zero, this.timeUnit) / tickSpacing) *
                        tickSpacing,
                    this.timeUnit,
                );
        } else {
            niceMin = min
                ...
                .set(
                    this.timeUnit,
                    Math.floor(min.get(this.timeUnit) / tickSpacing) *
                        tickSpacing,
                );
            niceMax = max
                .clone()
                .set(
                    this.timeUnit,
                    Math.ceil(max.get(this.timeUnit) / tickSpacing) *
                        tickSpacing,
                );
        }
        ...
        return xArr.map((value, index) => {
            return new LabeledTick<Date>(
                value.toDate(),
                this.tickHeight,
                index,
                this.getTimeUnitValueAsString(value),
            );
        });
    }

    protected chooseTicksSpacingOutOfOptions(
        spacingOptions: Array<number>,
        ticksDelta: number,
        ticksCount: number,
    ): number {
        ...
        return flow(
            partialRight(map, (spacingOption: number) => {
                return {
                    diff: spacingOption - dirtySpacing,
            ...
            partialRight(
                sortBy,
                (v: { diff: number; spacing: number }) => v.diff,
            ),
            partialRight(
                map,
                (v: { diff: number; spacing: number }) => v.spacing,
            ),
            first,
        )(spacingOptions);
    }
