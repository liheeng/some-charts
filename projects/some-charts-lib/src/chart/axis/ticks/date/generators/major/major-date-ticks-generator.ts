...
export class MajorDateTicksGenerator extends MajorTicksGenerator<Date> {
    ...
    constructor(tickHeight: number) {
        ...
        this.majorMillisecondsTicksGenerator =
            new MajorMillisecondsTicksGenerator(tickHeight);
        this.majorSecondsTicksGenerator = new MajorSecondsTicksGenerator(
            tickHeight,
        );
        this.majorMinutesTicksGenerator = new MajorMinutesTicksGenerator(
            tickHeight,
        );
        this.majorHoursTicksGenerator = new MajorHoursTicksGenerator(
            tickHeight,
        );
        this.majorDaysTicksGenerator = new MajorDaysTicksGenerator(tickHeight);
        this.majorMonthsTicksGenerator = new MajorMonthsTicksGenerator(
            tickHeight,
        );
        this.majorYearsTicksGenerator = new MajorYearsTicksGenerator(
            tickHeight,
        );
    }

    generateTicks(range: Range<Date>, ticksCount: number): Array<Tick<Date>> {
        return this.getSuitableTimeUnitTicksGenerator(range).generateTicks(
            range,
            ticksCount,
        );
    }

    getSuitableTimeUnitTicksGenerator(
        range: Range<Date>,
    ): MajorTimeUnitTicksGenerator {
