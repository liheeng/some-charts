...
import {
    ...
    MinorLvl1MinutesTicksGenerator,
    MinorLvl1MonthsTicksGenerator,
    MinorLvl1SecondsTicksGenerator,
    MinorLvl1TimeUnitTicksGenerator,
    MinorLvl1YearsTicksGenerator,
} from './lvl1';
import {
    MinorLvl2DaysTicksGenerator,
    MinorLvl2HoursTicksGenerator,
    MinorLvl2MinutesTicksGenerator,
    MinorLvl2MonthsTicksGenerator,
    MinorLvl2TimeUnitTicksGenerator,
    MinorLvl2YearsTicksGenerator,
} from './lvl2';
...
export class MinorDateTicksGenerator implements MinorTicksGenerator<Date> {
    ...
    constructor(
        tickHeight: number,
        majorTicksGenerator: MajorDateTicksGenerator,
    ) {
        this.tickHeight = tickHeight;

        this.majorTicksGenerator = majorTicksGenerator;

        this.minorLvl1SecondsTicksGenerator =
            new MinorLvl1SecondsTicksGenerator(tickHeight);
        this.minorLvl1MinutesTicksGenerator =
            new MinorLvl1MinutesTicksGenerator(tickHeight);
        this.minorLvl1HoursTicksGenerator = new MinorLvl1HoursTicksGenerator(
            tickHeight,
        );
        this.minorLvl1DaysTicksGenerator = new MinorLvl1DaysTicksGenerator(
            tickHeight,
        );
        this.minorLvl1MonthsTicksGenerator = new MinorLvl1MonthsTicksGenerator(
            tickHeight,
        );
        this.minorLvl1YearsTicksGenerator = new MinorLvl1YearsTicksGenerator(
            tickHeight,
        );

        this.minorLvl2MinutesTicksGenerator =
            new MinorLvl2MinutesTicksGenerator(tickHeight);
        this.minorLvl2HoursTicksGenerator = new MinorLvl2HoursTicksGenerator(
            tickHeight,
        );
        this.minorLvl2DaysTicksGenerator = new MinorLvl2DaysTicksGenerator(
            tickHeight,
        );
        this.minorLvl2MonthsTicksGenerator = new MinorLvl2MonthsTicksGenerator(
            tickHeight,
        );
        this.minorLvl2YearsTicksGenerator = new MinorLvl2YearsTicksGenerator(
            tickHeight,
        );
    }

    getSuitableLvl1TimeUnitTicksGenerator(
        range: Range<Date>,
    ): MinorLvl1TimeUnitTicksGenerator | undefined {
        let majorGeneratorTimeUnits =
            this.majorTicksGenerator.getSuitableTimeUnitTicksGenerator(
                range,
            ).timeUnit;
    ...
    getSuitableLvl2TimeUnitTicksGenerator(
        range: Range<Date>,
    ): MinorLvl2TimeUnitTicksGenerator | undefined {
        let majorGeneratorTimeUnits =
            this.majorTicksGenerator.getSuitableTimeUnitTicksGenerator(
                range,
            ).timeUnit;
        ...
        if (majorGeneratorTimeUnits === TimeUnit.Years) {
            ...
            if (
                Math.abs(max.get(TimeUnit.Months) - min.get(TimeUnit.Months)) >=
                1
            ) {
                return this.minorLvl2YearsTicksGenerator;
            } else {
            ...
            if (
                Math.abs(max.get(TimeUnit.Days) - min.get(TimeUnit.Days)) >= 1
            ) {
                return this.minorLvl2MonthsTicksGenerator;
            } else {
            ...
            if (
                Math.abs(max.get(TimeUnit.Hours) - min.get(TimeUnit.Hours)) >= 1
            ) {
                return this.minorLvl2DaysTicksGenerator;
            } else {
            ...
            if (
                Math.abs(
                    max.get(TimeUnit.Minutes) - min.get(TimeUnit.Minutes),
                ) >= 1
            ) {
                return this.minorLvl2HoursTicksGenerator;
            } else {
    ...
    generateTicks(
        range: Range<Date>,
        majorTicks: Array<Tick<Date>>,
    ): Array<Tick<Date>> {
        let lvl1Ticks =
            this.getSuitableLvl1TimeUnitTicksGenerator(range)?.generateTicks(
                range,
                majorTicks,
            ) ?? [];
        let lvl2Ticks =
            this.getSuitableLvl2TimeUnitTicksGenerator(range)?.generateTicks(
                range,
                majorTicks,
            ) ?? [];
