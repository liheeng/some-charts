...
export class PlotOptionsDefaults {
    public static extendWith(
        ...
        if (options.kind === PlotKind.BoxOutliers) {
            return BoxOutliersPlotOptionsDefaults.Instance.extendWith(
                options as BoxOutliersPlotOptions,
                skin,
            );
        } else if (options.kind === PlotKind.Box) {
            return BoxPlotOptionsDefaults.Instance.extendWith(
                options as BoxPlotOptions,
                skin,
            );
        } else if (options.kind === PlotKind.Marker) {
            return MarkerPlotOptionsDefaults.Instance.extendWith(
                options as MarkerPlotOptions,
                skin,
            );
            ...
            throw new Error(`Plot of ${options.kind} kind is not supported!`);
    }
}
