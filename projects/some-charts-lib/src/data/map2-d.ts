...
> {
    ...
    constructor(
        xy: readonly ValueType[],
        xExtractor: (v: ValueType) => number,
        yExtractor: (v: ValueType) => number,
    ) {
        ...
        for (let x of mapByX.keys()) {
            ...
            mapByXbyY.set(
                x,
                new Map(letValuesForX.map((v) => [yExtractor(v), v])),
            );
        }
    ...
    public has(
        x: DimensionValue<XDimensionType>,
        y: DimensionValue<YDimensionType>,
    ) {
