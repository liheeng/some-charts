...
> {
    ...
    static fromDataSet<
        ...
        YDimensionType extends number | string | Date | undefined = undefined,
    >(
        dataSet: DataSet<any, XDimensionType, YDimensionType>,
    ...
    static fromDataSetDimensionsUpdate<
        XDimensionType extends number | string | Date,
        YDimensionType extends number | string | Date | undefined = undefined,
    >(
        ...
        let id1D =
            (prevDimensionsValues.length !== 0 &&
                !Array.isArray(prevDimensionsValues[0])) ||
            (dimensionsValues.length !== 0 &&
                !Array.isArray(dimensionsValues[0]));
...
export class DataSetChange1D<
    XDimensionType extends number | string | Date,
> extends DataSetChange<XDimensionType> {
    ...
    constructor(
        deleted: readonly DimensionValue<XDimensionType>[],
        ...
        this.deletedMap = new Map(
            (deleted as DimensionValue<XDimensionType>[]).map((v) => [
                v.toNumericValue(),
                v,
            ]),
        );
        this.updatedMap = new Map(
            (updated as DimensionValue<XDimensionType>[]).map((v) => [
                v.toNumericValue(),
                v,
            ]),
        );
        this.addedMap = new Map(
            (added as DimensionValue<XDimensionType>[]).map((v) => [
                v.toNumericValue(),
                v,
            ]),
        );
    ...
    public static fromDataSetDimensionsUpdate1D<
        XDimensionType extends number | string | Date,
        YDimensionType extends number | string | Date | undefined = undefined,
    >(
        ...
        let prevDimensionValuesMap = new Map(
            prevDimensionValues.map((v) => [v.toNumericValue(), v]),
        );
        let dimensionValuesMap = new Map(
            dimensionValues.map((v) => [v.toNumericValue(), v]),
        );

        for (let prevDimVal of prevDimensionValues) {
            if (dimensionValuesMap.has(prevDimVal.toNumericValue())) {
                updated.push(
                    dimensionValuesMap.get(prevDimVal.toNumericValue())!,
                );
            } else {
...
> extends DataSetChange<XDimensionType, YDimensionType> {
    ...
    private readonly deletedMap: Map2D<
        XDimensionType,
    ...
    >;
    private readonly updatedMap: Map2D<
        XDimensionType,
        Exclude<YDimensionType, undefined>,
        [
            DimensionValue<XDimensionType>,
    ...
    >;
    private readonly addedMap: Map2D<
        XDimensionType,
        Exclude<YDimensionType, undefined>,
        [
            DimensionValue<XDimensionType>,
    ...
    public static fromDataSetDimensionsUpdate2D<
        XDimensionType extends number | string | Date,
        YDimensionType extends number | string | Date | undefined,
    >(
        prevDimensionsValues: [
        ...
        let prevDimensionsValueMap = new Map2D(
            prevDimensionsValues,
            (v) => v[0].toNumericValue(),
            (v) => v[1].toNumericValue(),
        );
        let dimensionsValuesMap = new Map2D(
            dimensionsValues,
            (v) => v[0].toNumericValue(),
            (v) => v[1].toNumericValue(),
        );
        ...
        for (let prevDimsVal of prevDimensionsValues) {
            if (dimensionsValuesMap.has(prevDimsVal[0], prevDimsVal[1])) {
                updated.push(
                    dimensionsValuesMap.get(prevDimsVal[0], prevDimsVal[1])!,
                );
            } else {
