...
> {
    ...
    private readonly dimensionYFunc:
    ...
    private _dimensionYValues?: DimensionValue<
        Exclude<YDimensionType, undefined>
    >[];
    private _dimensionsValues:
        | DimensionValue<XDimensionType>[]
        | [
              DimensionValue<XDimensionType>,
              DimensionValue<Exclude<YDimensionType, undefined>>,
          ][];
    ...
    public get dimensionYValues():
        | readonly DimensionValue<Exclude<YDimensionType, undefined>>[]
        | undefined {
        return this._dimensionYValues;
    }

    public get dimensionsValues():
        | readonly DimensionValue<XDimensionType>[]
        | readonly [
              DimensionValue<XDimensionType>,
              DimensionValue<Exclude<YDimensionType, undefined>>,
          ][] {
    ...
    private metricsRanges: Map<string, Range<number>> = new Map<
        string,
        Range<number>
    >();
    ...
    constructor(
        ...
        this.indexByYDimension = dimensionYFunc
    ...
    public getMetricRange(metricId: string) {
        ...
        if (!metricRange) {
            ...
            if (metricValues) {
                ...
                throw new Error(
                    "Failed to get metric value. Metric with specified name doesn't exist in this DataSet.",
                );
        }
    ...
    public getMetricValue(
        ...
        if (
            (!isUndefined(y) && !this.dimensionYFunc) ||
            (isUndefined(y) && this.dimensionYFunc)
        ) {
        ...
        let xIdx = this.indexByXDimension.get(
            new DimensionValue(x).primitiveValue,
        );

        if (metricValues) {
            if (!isUndefined(y)) {
                let yIdx = this.indexByYDimension!.get(
                    new DimensionValue(y).primitiveValue,
                );
                return !isUndefined(xIdx) && !isUndefined(yIdx)
                    ? (metricValues as Array<Array<number | Array<number>>>)[
                          xIdx
                      ][yIdx]
                    : undefined;
            } else {
                return !isUndefined(xIdx)
                    ? (metricValues as Array<number | Array<number>>)[xIdx]
                    : undefined;
            }
        } else
            throw new Error(
    ...
    public getMetricValueForDimensions(
        metricId: string,
        xDimVal: DimensionValue<XDimensionType>,
        yDimVal: DimensionValue<Exclude<YDimensionType, undefined>> | undefined,
    ): number | Array<number> | undefined {
        if (
            (yDimVal && !this.dimensionYFunc) ||
            (!yDimVal && this.dimensionYFunc)
        ) {
            throw new Error('Failed to get metric value. Dimensions mismatch.');
        }
        ...
        if (metricValues) {
            if (!isUndefined(xDimVal) && !isUndefined(yDimVal)) {
                return (metricValues as Array<Array<number | Array<number>>>)[
                    xDimVal.index
                ][yDimVal.index];
            } else {
                return (metricValues as Array<number | Array<number>>)[
                    xDimVal.index
                ];
            }
        } else
    ...
    public getMetricValues(
        metricId: string,
    ): Array<number | Array<number>> | undefined {
        return this.metricsValues.get(metricId);
    }

    public getSingleMetricValue(
        metricId: string,
        x: XDimensionType,
        y?: YDimensionType,
    ): number | undefined {
    ...
    public getArrayMetricValue(
        metricId: string,
        x: XDimensionType,
        y?: YDimensionType,
    ): Array<number> | undefined {
        if (this.isArrayMetricValue(metricId)) {
            return this.getMetricValue(metricId, x, y) as
                | Array<number>
                | undefined;
        } else throw DataSet.buildMetricIsNotArrayError(metricId);
    }

    public getSingleMetricValueForDimensions(
        metricId: string,
        xDimVal: DimensionValue<XDimensionType>,
        yDimVal: DimensionValue<Exclude<YDimensionType, undefined>> | undefined,
    ): number | undefined {
        if (this.isSingleMetricValue(metricId)) {
            return this.getMetricValueForDimensions(
                metricId,
                xDimVal,
                yDimVal,
            ) as number | undefined;
    ...
    public update(elements: Array<TItemType>) {
        ...
        this.dimensionXValues.forEach((dimXVal) => {
            if (
                typeof dimXVal.primitiveValue === 'number' &&
                (!dimensionXPrevMaxNumeric ||
                    dimXVal.primitiveValue > dimensionXPrevMaxNumeric)
            ) {
        ...
        if (this.dimensionYValues) {
            this.dimensionYValues.forEach((dimYVal) => {
                if (
                    typeof dimYVal.primitiveValue === 'number' &&
                    (!dimensionYPrevMaxNumeric ||
                        dimYVal.primitiveValue > dimensionYPrevMaxNumeric)
                ) {
        ...
        let dimensionXValuesMap = new Map<
            number | string,
            DimensionValue<XDimensionType>
        >(this.dimensionXValues.map((v) => [v.primitiveValue, v]));
        let dimensionYValuesMap = this.dimensionYFunc
            ? new Map<
                  number | string,
                  DimensionValue<Exclude<YDimensionType, undefined>>
        ...
        let dimensionsXYValues: [
            DimensionValue<XDimensionType>,
            DimensionValue<Exclude<YDimensionType, undefined>>,
        ][] = [];

        elements.forEach((element, index) => {
            let dimXValue = new DimensionValue(
                this.dimensionXFunc(element),
                index,
            );
            let dimYValue = this.dimensionYFunc
                ? new DimensionValue(this.dimensionYFunc!(element), index)
                : undefined;

            let errorTxt =
                'For numeric or Date dimensions, update operation can only add values larger than those already inside DataSet (e.g. extend time series data) or override existing data.';
        ...
        this._dimensionXValues = Array.from(dimensionXValuesMap.values())
            .filter(
                (v) =>
                    dimensionXMinNumeric === undefined ||
                    typeof v.primitiveValue !== 'number' ||
                    v.primitiveValue >= dimensionXMinNumeric,
            )
        ...
        this._dimensionYValues = dimensionYValuesMap
            ? Array.from(dimensionYValuesMap.values())
                  .filter(
                      (v) =>
                          dimensionYMinNumeric === undefined ||
                          typeof v.primitiveValue !== 'number' ||
                          v.primitiveValue >= dimensionYMinNumeric,
                  )
                  .sort(DimensionValue.getCompareFunc())
                  .map((dv, i) => dv.withIndex(i))
            : undefined;

        this.indexByXDimension = new Map(
            this._dimensionXValues.map((dv) => [dv.primitiveValue, dv.index]),
        );
        this.indexByYDimension = this._dimensionYValues
            ? new Map(
                  this._dimensionYValues.map((dv) => [
                      dv.primitiveValue,
                      dv.index,
                  ]),
              )
        ...
        if (this.dimensionYFunc) {
            dimensionsXYValues = dimensionsXYValues
                .filter((dv) => {
                    return (
                        (dimensionXMinNumeric === undefined ||
                            typeof dv[0].primitiveValue !== 'number' ||
                            dv[0].primitiveValue >= dimensionXMinNumeric) &&
                        (dimensionYMinNumeric === undefined ||
                            typeof dv[1].primitiveValue !== 'number' ||
                            dv[1].primitiveValue >= dimensionYMinNumeric)
                    );
                })
        ...
        let dataSetChange: DataSetChange<XDimensionType, YDimensionType> =
            DataSetChange.fromDataSetDimensionsUpdate(
                prevDimensionsValues,
                this._dimensionsValues,
            );

        let indexXOffset =
            dimensionXValuesMap.size - this._dimensionXValues.length;
        let indexYOffset =
            dimensionYValuesMap && this._dimensionYValues
                ? dimensionYValuesMap.size - this._dimensionYValues.length
                : undefined;

        for (let metricId in this.metrics) {
            let metricValues: Array<number | Array<number>> =
                this.metricsValues.get(metricId) ??
                Array(this._dimensionXValues.length);
                ...
                if (indexYOffset) {
                    for (let i = 0; i < metricValues.length; i++) {
                        if (Array.isArray(metricValues[i])) {
                            let metricValuesInnerArr = metricValues[
                                i
                            ] as Array<number>;
            ...
            elements.forEach((element, index) => {
                ...
                let yIdx =
                    this.indexByYDimension && dimYValue
                        ? this.indexByYDimension.get(dimYValue.primitiveValue)
                        : undefined;
                ...
                if (
                    !isUndefined(xIdx) &&
                    !isUndefined(yIdx) &&
                    Array.isArray(metricValues[xIdx])
                ) {
                    ...
                    if (Array.isArray(metricValue)) {
                        throw new Error(
                            "DataSet doesn't support 2D metrics with array of values",
                        );
                    }

                    twoDMetricValues[xIdx][yIdx] = metricValue as number;
                } else if (!isUndefined(xIdx)) {
                    let oneDMetricValues = metricValues as Array<
                        number | Array<number>
                    >;
        ...
        let firstDimXValue = this.dimensionXValues.length
            ? this.dimensionXValues[0].value
            : undefined;
        this._dimensionXType = this.getDimensionType(firstDimXValue);

        let secondDimXValue =
            this.dimensionYValues && this.dimensionYValues.length
                ? this.dimensionYValues[0].value
                : undefined;
    ...
    private clearInternal(triggerChange: boolean) {
        ...
        this.indexByXDimension = new Map<number | string, number>();
        this.indexByYDimension = this.dimensionYFunc
            ? new Map<number | string, number>()
            : undefined;
        if (triggerChange) {
            let dataSetChange = this.is1D
                ...
                : new DataSetChange2D(
                      this.dimensionsValues as readonly [
                          DimensionValue<XDimensionType>,
                          DimensionValue<Exclude<YDimensionType, undefined>>,
                      ][],
                      [],
    ...
    protected getDimensionType(
        firstDimValue: number | string | Date | undefined,
    ...
    xDimensionValueToNumeric(value: XDimensionType): number | undefined {
        return value
            ? new DimensionValue(
                  value,
                  this.indexByXDimension?.get(
                      DimensionValue.makePrimitive(value),
                  ),
              ).toNumericValue()
            : undefined;
    }
    ...
    numericToXDimensionValue(value: number): XDimensionType | undefined {
        if (this._dimensionXType === DimensionType.Date) {
            return DimensionValue.buildForDateFromPrimitive(value)
                .value as XDimensionType;
            ...
            if (this.dimensionXValues.length) {
                ...
                let maxX =
                    this.dimensionXValues[
                        this.dimensionXValues.length - 1
                    ].toNumericValue();
                ...
                if (this.is1D) {
                    ...
                    if (!isUndefined(minY) && !isUndefined(maxY)) {
                        curMetricBoundingRect = new NumericDataRect(
                            minX,
                            maxX,
                            minY,
                            maxY,
                    ...
                    if (this.dimensionYValues && this.dimensionYValues.length) {
                        let dimensionYNumericValues = this.dimensionYValues.map(
                            (v) => v.toNumericValue(),
                        );
